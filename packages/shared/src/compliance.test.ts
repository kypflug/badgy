...
import { trailingMondays } from './calendar.js';
...
describe('compliance', () => {
  ...
  it('officeDaysByWeek counts Mon–Fri office', () => {
    const office = officeDaysByWeek(emptyDoc(), trailingMondays('2026-06-15', 1), TODAY);
    expect(office[0]).toBe(5);
    expect(beltForWeek(emptyDoc(), '2026-06-15', TODAY)).toBeCloseTo(1, 10);
  });
});
