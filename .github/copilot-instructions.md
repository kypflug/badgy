...
- Style exclusively with Badgy tokens from `packages/web/src/styles/tokens.css`
  (`var(--badgy-…)`) in the global `app.css` — never hardcode a colour in a component. Slate
  Blueprint: cool drafting-paper light and slate-charcoal dark palettes on a steel-blue accent,
  8–14px radii (no `corner-shape`), a full-bleed pane, and one structural shadow on the inset
  rail/sign-in sheet: interior separation is hairlines and value steps. In Windows Controls Overlay
  mode, the rail brand occupies the titlebar and the existing pane header becomes the interactive
  WCO command bar; preserve native-control clearance and explicit drag/no-drag regions.
